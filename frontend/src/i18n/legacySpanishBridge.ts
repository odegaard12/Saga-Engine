...
const ES: Record<string, string> = {
  ...
  'Nodo sin título': 'Nodo sin título',
  'Basics': 'Básico',
  'Location': 'Ubicación',
  'Game': 'Juego',
  'Messages': 'Mensajes',
  ...
  'Objective': 'Objetivo',
  'Hint': 'Pista',
  'Live local preview · use Guardar in Mission Control to persist': 'Vista local en vivo · usa Guardar en Control de misión para persistir',

  'Players & teams': 'Jugadores y equipos',
  'Manage who can play this mission. Guardar players to persist changes.': 'Gestiona quién puede jugar esta misión. Guarda jugadores para persistir los cambios.',
  ...
  'Guardar players': 'Guardar jugadores',
  'Available family-native runtime labels.': 'Etiquetas disponibles del runtime nativo por familias.',
  ...
  'Guardar progress': 'Guardar progreso',
  ...
  'Auto-sync every 20s when pending events exist. Never synced.': 'Sincronización automática cada 20 s si hay acciones pendientes. Nunca sincronizado.',
  'Inventory': 'Inventario',
  'Items': 'Objetos',
  'items': 'objetos',
  'EMPTY': 'VACÍO',
  'No local items yet. QR/NFC/manual item collection can fill this panel next.': 'Aún no hay objetos. Puedes recogerlos con QR, NFC o código manual.',
  'updated now · local-first gameplay state': 'actualizado ahora · estado guardado en este dispositivo',
  ...
  '0 items': '0 objetos',
  '1 items': '1 objeto',
  'Auto-sync every 20s when pending events exist. Nunca sincronizado.': 'Sincronización automática cada 20 s si hay acciones pendientes. Nunca sincronizado.',
  'Open interaction': 'Abrir interacción',
  'Configure admin copy, player intro text and map defaults.': 'Configura textos del admin, introducción del jugador y valores iniciales del mapa.',
  'classic': 'clásico',
  'theme': 'tema',
  'Identity': 'Identidad',
  ...
  'Guardar settings': 'Guardar ajustes',
...
function translateDynamic(value: string): string | null {
  ...
  if (match) return `${match[1]} pendientes · ONLINE`

  match = normalized.match(/^(\d+)\s+items$/i)
  if (match) return `${match[1]} objetos`

  match = normalized.match(/^(\d+)\s*M\s+IN$/i) || normalized.match(/^(\d+)M\s+IN$/i)
  if (match) return `${match[1]} M CERCA`
...
function translateNode(text: Text) {
  ...
  text.nodeValue = `${prefix}${translated}${suffix}`
}
