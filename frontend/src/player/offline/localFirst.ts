...
export function emptyOfflineSnapshot(user: string): SagaOfflineSnapshot {
  return {
    user,
    sync_status: typeof navigator !== "undefined" && navigator.onLine === false ? "offline" : "online",
    updated_at: nowIso(),
    queued_events: [],
  };
}
...
export function queueOfflineEvent(
  ...
  return saveOfflineSnapshot({
    ...snapshot,
    sync_status: typeof navigator !== "undefined" && navigator.onLine === false ? "offline" : snapshot.sync_status,
    queued_events,
  });
}
...
export async function fetchGamePayloadLocalFirst<T = unknown>(
  ...
  try {
    ...
    const snapshot = setSyncStatus(user, typeof navigator !== "undefined" && navigator.onLine === false ? "offline" : "error");
