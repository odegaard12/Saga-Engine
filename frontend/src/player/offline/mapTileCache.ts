...
export async function prefetchMissionMapTiles(
  ...
  if (routePoints.length > 0 && center) {
    ...
    addBBoxTilesWithBudget(urls, routePoints, 14, Math.min(MISSION_AREA_RADIUS_KM, 25), 280, 'mission-z14')

    // Corredor ancho, no línea fina.
    addRouteCorridor(urls, routePoints, 15, ROUTE_CORRIDOR_KM, 1600, 280, 'corridor-z15')
    addRouteCorridor(urls, routePoints, 16, Math.min(ROUTE_CORRIDOR_KM, 4), 1100, 340, 'corridor-z16')
    addRouteCorridor(urls, routePoints, 17, Math.min(ROUTE_CORRIDOR_KM, 2.2), 850, 340, 'corridor-z17')
