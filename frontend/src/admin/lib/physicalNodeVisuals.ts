...
export function getPhysicalNodeKind(stage: unknown): PhysicalNodeKind | null {
  ...
  const isMapCollectible = Boolean(config.is_map_collectible || record.is_map_collectible)
  const hasRealQr = Boolean(record.physical_qr || record.qr_payload)
  const flatKind = normalizePhysicalKind(record.physical_node_kind || record.physical_item_kind)

  // Nodo con tarjeta/payload QR real: es un "Objeto QR" escaneable (🖨️),
  // aunque el objeto que entregue sea de tipo collectible. Llave/pista/bonus
  // conservan su etiqueta específica.
  if (hasRealQr && !isMapCollectible) {
    if (flatKind === 'requirement' || flatKind === 'clue' || flatKind === 'bonus') {
      return flatKind
    }
    ...
    if (physicalQr && typeof physicalQr === 'object') {
      ...
      if (qrKind === 'requirement' || qrKind === 'clue' || qrKind === 'bonus') {
        return qrKind
      }
    }
    return 'qr'
