...
export function hasExplicitQrMarker(stage: StageLike): boolean {
  // Solo cuentan marcadores de QR REAL: tarjeta guardada, payload, o kinds de
  // tarjeta física (llave/pista/bonus). Un coleccionable de mapa (kind
  // 'collectible' sin payload) NO es un nodo QR: se recoge por GPS.
  const kind = String(stage.physical_node_kind || stage.physical_item_kind || '')
  return Boolean(
    stage.physical_qr ||
    stage.qr_payload ||
    kind === 'requirement' ||
...
export function isMapCollectibleStage(stage: StageLike): boolean {
  ...
  // Un checkpoint nunca es coleccionable.
  if (isCheckpointStage(stage)) return false

  // Con tarjeta o payload QR real es un nodo QR escaneable, no un coleccionable.
  if (stage.physical_qr || stage.qr_payload) return false

  // Coleccionable de mapa explícito (se recoge por GPS, sin escanear).
  if (config.is_map_collectible || stage.is_map_collectible) return true

  // Nodo que entrega un objeto del catálogo (kind collectible sin QR).
  const kind = String(stage.physical_node_kind || stage.physical_item_kind || '')
  if (kind === 'collectible' && (stage.physical_item_id || stage.physical_item_label)) {
    return true
  }
...
export function isCheckpointStage(stage: StageLike): boolean {
  const config = configOf(stage)
  return (
...
export function isQrStage(stage: StageLike): boolean {
  ...
  // Un checkpoint nunca abre el editor de QR/coleccionable aunque el nodo
  // arrastre campos physical_* residuales de un tipo anterior.
  if (isCheckpointStage(stage)) return false
  const config = configOf(stage)
  const gameId =
