...
function getPhysicalMode(stage: AdminReactOverviewStage): NodePhysicalMode {
  ...
  // Checkpoint gana siempre: campos physical_* residuales no lo convierten en QR.
  if (
    config.game_id === 'simple_checkpoint' ||
    record.game_type === 'simple_checkpoint' ||
    record.game_template_id === 'simple_checkpoint'
  ) {
    return 'none'
  }
  ...
  // kind 'collectible' sin tarjeta/payload QR = coleccionable de mapa (GPS)
  if (raw === 'collectible' && !record.physical_qr && !record.qr_payload) {
    return 'map_collectible'
  }
...
export default function NodePhysicalTypePanel({
  ...
  function setMode(nextMode: NodePhysicalMode) {
    if (nextMode === 'none') {
      ...
      const clearedConfig =
        cleared.config && typeof cleared.config === 'object'
          ? (cleared.config as Record<string, unknown>)
          : {}

      const wasCheckpoint =
        clearedConfig.game_id === 'simple_checkpoint' ||
        cleared.game_type === 'simple_checkpoint' ||
        cleared.game_template_id === 'simple_checkpoint'

      if (wasCheckpoint) {
        // Al pasar de checkpoint a minijuego, aplicar un juego real por defecto
        // para que no se quede atrapado en la identidad de checkpoint.
        const baseGame = getDefaultAdminStagePatchForGame('logic_circuit')
        onApplyLocal({
          ...cleared,
          ...baseGame,
          title: stage.title || baseGame.label,
          game_type: 'logic_circuit',
          game_template_id: 'logic_circuit',
          game_family: baseGame.type,
      ...
      onApplyLocal(cleared as unknown as AdminReactOverviewStage)
      onFinishChoice?.()
      return
    }
