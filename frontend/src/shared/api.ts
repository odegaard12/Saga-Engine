...
async function postJsonLoose(url: string, body: unknown): Promise<unknown> {
  const res = await fetch(url, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
...
function adminPayloadVariants(password: string, extra: Record<string, unknown> = {}) {
  return [
    { password, ...extra },
...
function normalizeAdminStagesPayload(payload: unknown): AdminStagesResponse {
...
function normalizeAdminSavePayload(payload: unknown): AdminSaveResponse {
  if (!payload || typeof payload !== 'object') {
    return { status: 'ok' }
  }
...
export async function fetchAdminStages(password: string): Promise<AdminStagesResponse> {
  ...
  for (const body of adminPayloadVariants(password)) {
    try {
      const payload = await postJsonLoose('/api/admin/stages', body)
      const normalized = normalizeAdminStagesPayload(payload)

      if (normalized.status === 'ok') {
        return normalized
      }

      errors.push(normalized.message || 'Unknown stages response error.')
    } catch (err) {
      errors.push(err instanceof Error ? err.message : 'Unknown stages request error.')
    }
  }
...
export async function saveAdminStages(
  ...
  const payloads = [
    ...adminPayloadVariants(password, { stages }),
    ...adminPayloadVariants(password, { data: stages }),
  ]

  for (const body of payloads) {
    try {
      const payload = await postJsonLoose('/api/admin/save', body)
      const normalized = normalizeAdminSavePayload(payload)
