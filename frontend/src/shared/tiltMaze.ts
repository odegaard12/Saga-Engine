...
export function nextTiltMazeCell(
  maze: GeneratedTiltMaze,
  index: number,
  direction: TiltDirection,
) {
  const spec = DIRECTIONS.find(
...
export function generateTiltMaze({
  ...
  const safeRows = Math.max(
    5,
    Math.min(13, Math.round(rows)),
  )

  const safeCols = Math.max(
    5,
    Math.min(13, Math.round(cols)),
  )

  const total = safeRows * safeCols
  const random = randomFor(
    `${seed}:${safeRows}:${safeCols}`,
  )

  const cells: TiltMazeCell[] =
    Array.from(
      { length: total },
      () => ({
        walls:
  ...
  const collectibles =
    collectibleCandidates.slice(
      0,
      Math.max(
        0,
        Math.min(
          collectibleCandidates.length,
          Math.round(
            collectibleCount,
          ),
        ),
