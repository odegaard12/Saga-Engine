...
def check_minigame_normalization() -> None:
    ...
    # Simón Dice. La config antigua describía un puzle de ORDENAR fichas
    # ("sequence", "shuffle_choices") que nunca se jugó: el runtime siempre fue
    # de colores. El contrato comprueba ahora los ajustes reales.
    ...
    assert_equal(sequence_code["levels"], 5, "simon levels")
    ...
    assert_equal(fuera_de_rango["levels"], 8, "simon levels clamped")
    assert_equal(fuera_de_rango["pad_count"], 3, "simon pads clamped")
    assert_equal(fuera_de_rango["step_ms"], 260, "simon step clamped")
...
def check_stage_runtime_contract(main) -> None:
    ...
    assert_equal(
        sequence_runtime["config"]["levels"],
        5,
        "simon runtime levels",
    )

    assert_equal(
        sequence_runtime["config"]["pad_count"],
        4,
        "simon runtime pad count",
    )
