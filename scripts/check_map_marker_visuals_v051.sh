...
[ "$(sed -n "s/^const PLAYER_SHELL_CACHE = '\([^']*\)'/\1/p" frontend/src/player/offline/pwaShell.ts)" = "saga-player-shell-v517-tools-qr-studio" ]
[ "$(sed -n "s/^const CACHE_NAME = '\([^']*\)'/\1/p" frontend/public/sw.js)" = "saga-player-shell-v517-tools-qr-studio" ]
