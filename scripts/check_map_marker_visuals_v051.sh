...
[ "$(sed -n "s/^const PLAYER_SHELL_CACHE = '\([^']*\)'/\1/p" frontend/src/player/offline/pwaShell.ts)" = "saga-player-shell-v518-tools-copy-cleanup" ]
[ "$(sed -n "s/^const CACHE_NAME = '\([^']*\)'/\1/p" frontend/public/sw.js)" = "saga-player-shell-v518-tools-copy-cleanup" ]
