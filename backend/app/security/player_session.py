...
def verify_player_session_token(token: str | None, *, user: str, secret: str, now: int | None = None) -> bool:
    ...
    if not payload:
        return False
    # En bytes, no en texto.
    #
    # hmac.compare_digest revienta con un TypeError si la cadena lleva algo
    # fuera del ASCII, y uno de los jugadores se llama Alvaro con tilde: al
    # completar un nodo el servidor devolvia un 500 y ese jugador no podia
    # avanzar en toda la ruta. Comparando los bytes se sigue tardando lo mismo
    # mire lo que mire —que es para lo que sirve esta funcion— y las tildes
    # dejan de importar.
    esperado = str(payload.get("user") or "").encode("utf-8")
    recibido = str(user or "").strip().encode("utf-8")
