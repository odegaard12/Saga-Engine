...
async def get_game_payload(user: str, request: Request, offline_pack: bool = False):
    ...
    payload = {
        ...
        "profile": profile,
        "live_status": main.project_live_profile_status(profile, live_positions.get(profile_id)),
        "level": lvl,
        "finished": finished,
        "stages": stages,
...
async def get_team_payload(user: str):
    ...
    for profile in main.get_player_profiles(cfg):
        projected = main.project_live_profile_status(profile, live_positions.get(profile.get("id")), now)
...
async def advance(request: Request):
    ...
    if lvl < len(stages):
        ...
        if main.stage_accepts_code(current_node, code):
