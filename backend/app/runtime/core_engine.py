...
def _normalize_item_requirement(raw):
    """Rellena las claves que quien consume el requisito da por hechas.

    Antes esta función devolvía tal cual el primer elemento de
    requirements.items. Un nodo cuyo requisito se escribiera sin "consume" —lo
    normal si lo genera una herramienta o se edita a mano— hacía que
    evaluate_stage_item_requirement reventara con KeyError: 'consume', y
    /api/advance respondía 500. El jugador no veía ningún error: el cliente
    caía a su copia local, daba el nodo por bueno y la partida avanzaba sólo en
    el móvil, mientras el servidor lo dejaba en el nodo anterior.
    """
    item_id = str(raw.get("item_id") or raw.get("required_item_id") or "").strip()
    if not item_id:
        return None

    return {
        "item_id": item_id,
        "quantity": _positive_int(raw.get("quantity") or raw.get("required_item_quantity"), 1),
        "consume": bool(raw.get("consume") or raw.get("required_item_consume") or False),
...
def read_stage_item_requirement(raw_stage):
    ...
    if isinstance(req, dict):
        ...
        if isinstance(items, list) and items and isinstance(items[0], dict):
            normalized = _normalize_item_requirement(items[0])
            if normalized:
                return normalized
