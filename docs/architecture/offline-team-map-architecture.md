...
The main server can live behind a public HTTPS domain such as `saga.example.org`, but routes may happen far away from the server. If the player has no mobile data, the phone cannot reach the remote backend in real time.
...
https://saga.example.org/scan/node-03-x7k9
